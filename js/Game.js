class Game {
  ...
  play(){
    ...
    if(allPlayers !== undefined){
      ...
      if(frameCount%70==0){
        var coin = createSprite(random(0,displayWidth),random(0,displayHeight+1000),10,10);
      ...
      if(frameCount%120==0){
        var coin1 = createSprite(random(0,displayWidth),random(0,displayHeight+1000),10,10);
     ...
     drawSprites()
        ...
        //  text(player.name + ": " + cars.car4.score, displayWidth/2, displayHeight/2);
            ...
            if(keyCode === 32){
              ...
              bullets = createSprite(player.x,player.y,10,5);
              bullets.velocityY = -3;
        } 

        if(bullets.isTouching(player)){
          player.destroy();
          text("YOU LOST", displayWidth/2, 2000);
        }
      }
